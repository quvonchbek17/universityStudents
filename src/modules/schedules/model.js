...
const postLesson = async(name, teacher, room, day, startTime, groupId) => {
   const created = await fetchData(`Select * from schedules where lesson_name = $1 and lesson_teacher = $2 and lesson_room = $3 and lesson_day = $4 and start_time = $5 and group_id = $6`, name, teacher, room, day, startTime, groupId)
   if(created.length > 0){
    return null
   } else {
...
const updateLesson = (id, name, teacher, room, day, startTime, groupId) => fetchData(UPDATE_LESSON, id, name, teacher, room, day, startTime, groupId)
